...
describe("AddContactForm component", () => {
  ...
  it(
    "should pass `contact` to `contactAdded` binding when submitting",
    angular.mock.inject(($componentController: any) => {
      const bindings = {
        contactAdded: jasmine.createSpy("contactAdded")
      };
      ...
      expect(bindings.contactAdded).toHaveBeenCalledWith({
        $event: { contact: { firstName: "John", lastName: "Doe" } }
      });
    })
  );

  it(
    "should clear `firstName` and `lastName` when submitting",
    angular.mock.inject(($componentController: any) => {
      const bindings = {
        contactAdded: jasmine.createSpy("contactAdded")
      };
