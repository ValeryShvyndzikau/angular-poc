...
describe("ContactList component", () => {
  beforeEach(() => {
    angular.module("app", []).component(ContactList.selector, ContactList);
  ...
  it(
    "should pass `contact` to `contactRemoved` binding when removing",
    angular.mock.inject(($componentController: any) => {
      const bindings = {
      ...
      expect(bindings.contactRemoved).toHaveBeenCalledWith({
        $event: { id: 1 }
      });
    })
  );
